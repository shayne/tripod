...
class Timer(threading.Thread):
    ...
    def run_later(self, callable_, timeout, *args, **kwargs):
        ...
        try:
            ...
            self._jobs.append((job, time.time() + timeout, timeout))
    ...
    def _get_sleep_time(self):
        if not self._jobs:
            ...
            job, scheduled_at, _ = self._jobs[0]
        ...
        while True:
            ...
            try:
                if not self._jobs:
                    ...
                    job, timeout, interval = self._jobs.pop(0)
                    self._jobs.append((job, time.time() + interval, interval))
                    self._jobs.sort(key=lambda job: job[1])  # sort on time
                    self.lock.notify()
            finally:
                self.lock.release()
